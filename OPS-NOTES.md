...
公开说明应直接记录这项取舍的适用条件：当前数据量小、桌面分发要求低依赖、
客户资料必须隔离，因此本地可解释检索优于远程向量库；数据规模变化后再重新评估。
...
- DeepSeek、OpenAI / Kimi 兼容服务、智谱和火山视觉服务应使用独立的低额度 Key；演示与日常使用不要共用高权限凭据。
- Key 建议按 30～90 天轮换并设置日/月消费上限。任何已进入终端输出、截图、日志或 Git 历史的 Key 都按泄露处理，先在服务商后台吊销，再清理文件与历史；只删 Git 内容不能恢复旧 Key 的安全性。
...
## 16. 公开展示材料约定（2026-08-19）

- README 产品截图只能从隔离演示环境生成，不得读取源码运行目录中的 `data/config.json`、客户文件、邮件历史、照片或日志。
- 演示机构、联系人、邮箱、号码和产品统一使用虚构名称与 `.example` 域名；截图生成后还要人工检查标题栏、路径、通知和浏览器信息。
- README 公开描述真实的设计决策、代码入口和可验证边界，不使用“面试题”“面试叙事”“可追问点”等求职包装措辞。
- 私人的面试准备材料与公开产品文档分离，不提交到本仓库；公开文档只保留能由代码、测试或构建产物支持的陈述。
